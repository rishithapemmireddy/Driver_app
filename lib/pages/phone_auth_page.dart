...
class _PhoneAuthPageState extends State<PhoneAuthPage> {
  ...
  bool _isKeypadVisible = false; // Controls keypad visibility
  ...
  Future<void> _verifyPhone() async {
    ...
    setState(() {
      _isLoading = true;
      _isKeypadVisible = false; // Hide keypad when processing
    });
    ...
    try {
      if (kIsWeb) {
        ...
        if (mounted) {
          Navigator.pushNamed(context, '/otp_verify', arguments: confirmationResult);
        }
      } else {
        await _auth.verifyPhoneNumber(
          ...
          verificationCompleted: (PhoneAuthCredential credential) async {
            ...
            if (mounted) Navigator.pushNamed(context, '/profile_details');
          },
          ...
          codeSent: (String verificationId, int? resendToken) {
            if (mounted) {
              Navigator.pushNamed(context, '/otp_verify', arguments: verificationId);
            }
          },
      ...
      if (e.code == 'billing-not-enabled') {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
            content: Text("SMS limit reached. Please use the Test Number configured in Firebase Console."),
      ...
      body: RawKeyboardListener(
        ...
        onKey: (event) {
          if (event is RawKeyDownEvent) {
            if (event.logicalKey == LogicalKeyboardKey.backspace) _handleBackspace();
            String? char = event.character;
            if (char != null && RegExp(r'^[0-9]$').hasMatch(char)) _handleInput(char);
          }
        },
        child: Container(
          ...
          child: SafeArea(
            child: Column(
              children: [
                  ...
                  child: SingleChildScrollView(
                    ...
                    child: Column(
                      ...
                      children: [
                        ...
                        Text("Enter your mobile number to continue", style: TextStyle(fontSize: 15, color: secondaryText)),
                        const SizedBox(height: 40),
                        Text("Mobile Number", style: TextStyle(fontSize: 14, color: secondaryText, fontWeight: FontWeight.w600)),
                        const SizedBox(height: 10),
                        GestureDetector(
                          onTap: () {
                            setState(() {
                              _isKeypadVisible = true; // Show keypad on tap
                            });
                          ...
                          child: Container(
                            ...
                            child: Row(
                              children: [
                                ...
                                const Text("+91", style: TextStyle(fontSize: 17, fontWeight: FontWeight.w600)),
                  ...
                  child: SizedBox(
                    ...
                    child: ElevatedButton(
                      ...
                      child: _isLoading 
                        ...
                        : const Text("Send OTP", style: TextStyle(fontSize: 18, color: Colors.white, fontWeight: FontWeight.w600)),
                    ),
                  ),
                ),
                ...
                const SizedBox(height: 15),
                // Only show custom keypad if _isKeypadVisible is true
                if (_isKeypadVisible)
                  CustomKeypad(onKeyPress: _handleInput, onBackspace: _handleBackspace),
              ],
            ),
          ),
        ),
...
class _StaticCursorState extends State<StaticCursor> {
  ...
  @override
  Widget build(BuildContext context) {
    return Opacity(opacity: _visible ? 1.0 : 0.0, child: Container(width: 2, height: 22, color: const Color(0xFF154FB9)));
  }
}
